...
import static hello.config.security.token.jwt.JWTConstants.HEADER_STRING;
import static hello.config.security.token.jwt.JWTConstants.TOKEN_PREFIX;
...
public class JWTAuthorizationFilter extends BasicAuthenticationFilter {
    ...
    private UsernamePasswordAuthenticationToken getAuthentication(HttpServletRequest request) throws IOException {
        ...
        if (token != null) {
            User user = ManageAuthToken.checkTokenAndReturnUser(token, userRepository);
            if (user != null) {
